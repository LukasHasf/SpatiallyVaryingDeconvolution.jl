...
function makemodel(psfs)
    ...
    elseif ndims(psfs)==4
        modelUNet = UNet3D.Unet3D(nrPSFs, 1, up="nearest", activation="relu", residual=true, norm="None")
    end
...
function train_model(model,
    ...
    for epoch in 1:(epochs-epoch_offset)
        ...
        if plotevery > 0 && epoch % plotevery == 0
            ...
            plot_prediction(pred_to_plot,
            psf_to_plot , epoch, epoch_offset, plotdirectory)
            if plotloss
...
function start_training(options_path; T=Float32)
    ...
    x_data, y_data = load_data(nrsamples, truth_directory, simulated_directory, newsize=newsize)
    ...
    if isnothing(loadpath)
        ...
        for i in 1:nrPSFs
            selectdim(resized_psfs, dims+1, i) .= imresize(collect(selectdim(psfs, dims+1, i)), newsize)
        end
    ...
    # Training
    train_model(model, train_x, train_y, test_x, test_y, loss_fn, epochs=epochs, checkpointdirectory=checkpoint_dir,
                plotloss=true, plotevery=plotevery, optimizer=optimizer, plotdirectory=plotpath, saveevery=saveevery)
end
